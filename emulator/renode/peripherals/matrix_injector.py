...
- Firmware drives one row pin LOW at a time.
- Firmware reads the column pins.
- A pressed switch at (r, c) connects col c to row r through a diode
  pointing row→col. When row r is low, col c reads low.
- Other rows (high) cannot pull cols low even if their switches are
