...
/* Debug LED trace for module loading. LED 0:
 * red = entry, green = validation OK, blue = SRAM write OK,
 * white = init called, off = init returned, cyan = load complete */
static inline void _mod_led(uint8_t r, uint8_t g, uint8_t b) {
    extern void debug_led_on(int led, uint8_t r, uint8_t g, uint8_t b);
    debug_led_on(0, r, g, b);
}
...
static bool module_install_hooks_and_init(uint8_t slot_id, uint32_t slot_addr,
    ...
    if (hdr->init_off > 0) {
        ...
        _mod_led(200, 200, 200);  // white = about to call init
        uint32_t rc = init_fn(module_init_env());
        _mod_led(0, 0, 0);  // off = init returned
...
static bool module_load_sram(uint8_t slot_id, const uint8_t* data, size_t len) {
    ...
    _mod_led(0, 200, 0);  // green = validation OK
    ...
    _mod_led(0, 0, 200);  // blue = SRAM write OK

    if (!module_install_hooks_and_init(slot_id, slot_addr, hdr, "mod load sram")) {
        module_sram_clear(slot_id);
        return false;
    }
    _mod_led(0, 200, 200);  // cyan = load complete
